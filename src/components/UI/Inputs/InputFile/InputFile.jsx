...
const InputFile = ({ register, errors }) => {
    ...
    const { ref: registerRef, ...rest } = register('photo', {
        ...
        validate: {
            validSize: (v) => validateImageSize(v) || `The file size should not exceed 5 MB`,
            validRes: async (v) => await validateImageResolution(v) || 'The image resolution should be at least 70px*70px',
        },
    });
    ...
    return (
        <>
            <Box className='input-file_wrapper'>
                ...
                <Box
                    className={`input-file-btn__descr ${loaded ? "loaded" : ""} ${errors?.photo ? 'input-file-btn__descr_error' : ''}`}
                    ref={inputDescrRef}
                    sx={{
